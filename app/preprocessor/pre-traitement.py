...
def apply_unit_conversions(df):
    ...
    df = df.drop(*[col_name for col_name in conversions if col_name in df.columns])
    df = df.withColumn("duration_minutes_accident", round((unix_timestamp("End_Time") - unix_timestamp("Start_Time")) / 60, 2))
    df = df.withColumn("duration_minutes_record_weather", round((unix_timestamp("Weather_Timestamp") - unix_timestamp("Start_Time")) / 60, 2))
    return df.drop("Start_Time", "End_Time", "Weather_Timestamp")

def transform_categorical_features(df):
    df = df.fillna({"Timezone": "UNKNOWN", "State": "UNKNOWN", "Weather_Condition": "UNKNOWN"})
    indexers = [
    ...
    return df.drop("Timezone", "State", "Weather_Condition")
...
def clean_wind_direction(df):
    ...
    df = df.withColumn("wind_angle", when(col("Wind_Direction_clean") == "N", 0.0)
    ...
    df = df.withColumn("wind_dir_sin", sin(radians(col("wind_angle")))) \
           .withColumn("wind_dir_cos", cos(radians(col("wind_angle")))) \
           .fillna({"wind_dir_sin": 0.0, "wind_dir_cos": 0.0})
    return df.drop("Wind_Direction", "wind_angle", "Wind_Direction_clean")
...
def transform_binary_features(df):
    for col_name in [
        ...
        df = df.withColumn(col_name + "_num", when(col(col_name) == True, 1).otherwise(0))
    df = df.drop("Amenity", "Bump", "Crossing", "Give_Way", "Junction", "No_Exit", "Railway", "Roundabout", "Station", "Stop", "Traffic_Calming", "Traffic_Signal", "Turning_Loop")

    for col_name in ["Sunrise_Sunset", "Civil_Twilight", "Nautical_Twilight", "Astronomical_Twilight"]:
        df = df.withColumn(col_name + "_num", when(col(col_name).isin(['day', 'Day']), 1)
                           .when(col(col_name).isin(['night', 'Night']), 0)
                           .otherwise(-1))
    return df.drop("Sunrise_Sunset", "Civil_Twilight", "Nautical_Twilight", "Astronomical_Twilight")
...
def manage_null_values(df):
    ...
    for col_name in numeric_mean_cols:
        df = df.fillna({col_name: df.select(mean(col(col_name))).first()[0]})
        df = df.withColumn(col_name, spark_round(col(col_name), 2))
    for col_name in numeric_zero_cols:
        df = df.fillna({col_name: 0.0}).withColumn(col_name, spark_round(col(col_name), 2))
    for col_name in numeric_median_cols:
        df = df.fillna({col_name: df.select(percentile_approx(col_name, 0.5)).first()[0]})
        df = df.withColumn(col_name, spark_round(col(col_name), 2))
    return df

def drop_columns(df, columns_to_drop):
    return df.drop(*[col for col in columns_to_drop if col in df.columns])

def inspect_last_bronze_and_save_to_hive(spark):
    ...
    try:
        ...
        df = drop_columns(df, ["ID", "Description", "situation_date", "Airport_Code", "Source", "Street", "City", "Country", "County", "Start_Lat", "Start_Lng", "End_Lat", "End_Lng", "Start_Time", "End_Time", "Zipcode"])
        df = transform_binary_features(df)
        df = manage_null_values(df)

        hive_table_name = "default.preprocessed_accidents_cleaned"
        df.write.mode("overwrite").format("parquet").saveAsTable(hive_table_name)
        logging.info(f" Données enregistrées dans la table Hive : {hive_table_name}")
...
inspect_last_bronze_and_save_to_hive(spark)
setup_logger()

spark.stop()
logging.info("Fin de lanalyse pré-traitement")
